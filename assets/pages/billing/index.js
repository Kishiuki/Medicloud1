...
$tableData = $("#dataPasiendiperiksa").DataTable({
	...
	ajax: {
			...
			data: function(d) {
				...
				d.asuransi=$("#filter_asuransi").val()
...
$(document)
	.ready(function () {
		...
		// $("#filter_pemeriksaan").select2()
	})
	.on("click", "[name='cara_pembayaran']", function () {
		var value = $(this).val()
		var nominal = parseFloat($(".sisa").text())
		if (value.toLowerCase() == 'asuransi') {
			$("#biaya").val(nominal).trigger('input')
		} else {
			$("#biaya").val('').focus()
			$("#kembalian").val('')
		}
			...
			$.each(data, function (key, val) {
				...
				if (key == 'id_asuransi') {
					if (val != '' && val != null && val != 0) {
						$("[name='cara_pembayaran'][value='Asuransi']").trigger('click').closest('.btn').removeClass('hide')
						$("#biaya").val(sisa.sisa)
						$("#kembalian").val(0)
					} else {
						$("[name='cara_pembayaran'][value='Asuransi']").closest('.btn').addClass('hide')
					}
				}
			})
			$.each(data, function (key, val) {
