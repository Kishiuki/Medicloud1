...
$(document)
  ...
  .on("click", ".link-history", function () {
    http_request("rawat-jalan/data-pasien/search_rm", "GET", { id: $(this).data("id") }).done(function (result) {
      var pem = '<tr style="background-color:lightgrey;">' + '<th width="15%">Kunjungan</th>' + '<th width="50%">Pemeriksaan</th>' + '<th width="15%">Diagnosis</th>' + '<th width="20%">Tindakan</th>' + "</tr>";
      $.each(result.pemeriksaan, function (index, item) {
        ...
        pem +=
          ...
          "<td>" +
          (item.status_rawat == 1 || item.status_rawat == 0 ? "<div class='tooltip-inner' style='background-color: peru;'>Rawat Jalan</div>" : "<div class='tooltip-inner' style='background-color: cornflowerblue;'>Rawat Inap</div>") +
          item.tanggal_kunjungan +
          "<br>" +
          "</td>" +
          '<td><table width="100%">' +
          '<tr><td width="15%">Sistole</td><td width="5%"> : </td><td width="30%">' +
          sistole +
          ' mm/Hg</td><td width="15%">Suhu tubuh</td><td width="5%"> : </td><td width="30%">' +
          suhu_tubuh +
          " ⁰C</td></tr>" +
          "<tr><td>Diastole</td><td> : </td><td>" +
          diastole +
          " mm/Hg</td><td>Saturasi</td><td> : </td><td>" +
          saturasi +
          " mmHg</td></tr>" +
          "<tr><td>Tensi</td><td> : </td><td>" +
          tensi +
          " mm/Hg</td><td>Nyeri</td><td> : </td><td>" +
          nyeri +
          "</td></tr>" +
          "<tr><td>Derajat nadi</td><td> : </td><td>" +
          derajat_nadi +
          " ppm</td><td>Berat bada</td><td> : </td><td>" +
          bb +
          " Kg</td></tr>" +
          "<tr><td>Nafas</td><td> : </td><td>" +
          nafas +
          " bpm</td><td>Tinggi badan</td><td> : </td><td>" +
          tb +
          " Cm</td></tr>" +
          '<tr style="vertical-align: top;"><td  width="25%">Anamnesa </td><td  width="5%"> : &nbsp;</td><td colspan="4"  width="70%">' +
          anamnesa +
          "</td></tr>" +
          '<tr style="vertical-align: top;"><td>Pemeriksaan umum</td><td> : </td><td colspan="4">' +
          pemeriksaan_umum +
          "</td></tr>" +
          '<tr style="vertical-align: top;"><td>Alergi </td><td> : </td><td colspan="4">' +
          alergi +
          "</td></tr>" +
          '<tr style="vertical-align: top;"><td>Catatan dokter</td><td> : </td><td colspan="4">' +
          catatan_dokter +
          "</td></tr>" +
          "</table></td>" +
          "<td>- " +
          diagnosa.split("|").join(",<br>- ") +
          "</td>" +
          "<td>- " +
          tindakan.split("|").join(",<br>- ") +
          "</td>" +
          "</tr > ";
      });
  ...
  .on("click", ".link-resep", function () {
    var id = $(this).data("id");
    http_request("rawat-jalan/pasien_telah_diperiksa/search_", "GET", { id: id }).done(function (result) {
      var data = result.data;
      $modal_id = $modalManageResep;
      $modal_body = $($modal_id).find(".modal-body");
      $modal_body.find("#id_pendaftaran").val(id);
  ...
  .on("click", ".link-edit-pemeriksaan", function () {
    http_request("rawat-jalan/pasien_telah_diperiksa/search_pemeriksaan", "GET", { id: $(this).data("id") }).done(function (result) {
      var data = result.data;
      $modal_id = $modalManagePeriksa;
      $modal_header = $($modal_id).find(".modal-header");
      $modal_body = $($modal_id).find(".modal-body");
      ...
      $modal_header.find("button.close").on("click", function () {
        location.reload();
      }); // codingan refresh
      if (result.dokter_diagnosa) {
        var selectdokter = $("#select_dokter");
        $.each(result.dokter_diagnosa, function (i, d) {
          var option = new Option(d.text, d.id, true, true);
          selectdokter.append(option).trigger("change");
          selectdokter.trigger({
            type: "select2:select",
            params: {
              data: option,
            },
      ...
      if (result.dokter_tindakan) {
        var selectdoktertindakan = $("#select_dokter_tindakan");
        $.each(result.dokter_tindakan, function (i, d) {
          var option = new Option(d.text, d.id, true, true);
          selectdoktertindakan.append(option).trigger("change");
          selectdoktertindakan.trigger({
            type: "select2:select",
            params: {
              data: option,
            },
  ...
  .on("click", ".link-pindah", function () {
    ...
    bootbox.confirm({
      ...
      callback: function (result) {
        if (result) {
          http_request("rawat-jalan/pasien_telah_diperiksa/pindah_ranap/" + id, "GET", {}).done(function (res) {
            Msg.success(res.message);
  ...
  .on("click", ".link-tambah-daftar-obat", function () {
    http_request("farmasi/penjualan/search_obat", "GET", { id: $(this).data("id") }).done(function (result) {
      var data = result.data;
      $modal_id = $modalManageResep;
      $modal_body = $($modal_id).find(".modal-body");
      $("#satuanobat").val("xxxx").trigger("change");
      $.each(data, function (key, val) {
        $modal_body.find("[name='" + key + "']").val(val);
      });
...
$("#satuanobat")
  ...
  .on("change", function () {
    ...
    http_request("farmasi/penjualan/search_obat_detail", "GET", { id: id }).done(function (result) {
      var data = result.data;
      $modal_id = $modalManageResep;
      $modal_body = $($modal_id).find(".modal-body");
...
function load_data_temp(id = "") {
  $.ajax({
    type: "GET",
    url: "pasien_telah_diperiksa/load_temp?clinic_id=" + getSelectedClinic() + "&id=" + id,
    data: "",
...
function hapus(id) {
  http_request("rawat-jalan/pasien_telah_diperiksa/hapus-temp", "POST", { id: id }).done(function (result) {
    $("#dataobat" + id).hide(200);
    load_data_temp($("#id_pendaftaran").val());
    $tableData.ajax.reload();
  });
}
function add_barang() {
  ...
  if ($("#satuanobat").val() == "" || $("#satuanobat").val() == null) {
    Msg.error("Satuan pembelian belum dipilih");
    return false;
  }
  if (qty == "" || parseInt(qty) < 1) {
    Msg.error("Qty wajib diisi");
    return false;
  }
  if (obat == "") {
    Msg.error("Nama obat wajib diisi");
    return false;
  }
  if (aturan_pakai == "") {
    Msg.error("Aturan pakai belum dipilih");
    return false;
  }
  if (cara_pakai == "") {
    Msg.error("Cara pakai belum dipilih");
    return false;
  }

  http_request("rawat-jalan/pasien_telah_diperiksa/insert-temp", "POST", {
