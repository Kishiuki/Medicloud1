...
$(document)
	...
	.on("change", "#filter_asuransi", function () {
	...
	.on("click",".link-history",function(){
		...
		'</tr>'
			$.each(result.pemeriksaan, function (index, item) {
				...
				var resep = (item.resep !== null && item.resep != '') ? item.resep : '';
...
$("#filter_asuransi").select2({
	...
	allowClear: true,
	ajax: { 
	url: base_url('pendaftaran/select2_asuransi/with-no'),
	type: "GET",
	dataType: 'json',
	delay: 250,
	data: function (params) {
