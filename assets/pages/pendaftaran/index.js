	...
	.on("click", ".link-tambah-daftar-pasien", function () {
		http_request('kunjunganIGD/search_pasien', 'GET', { id: $(this).data('id') })
			.done(function (result) {
				...
				$('#jenis_kelamin').val(data.jenis_kelamin).trigger('change').select2("enable",false)
				$('#status_nikah').html('<option value="' + data.status_nikah + '">' + data.status_nikah + '</option>').trigger('change').select2("enable",false);
				$('#agama').html('<option value="' + data.agama + '">' + data.agama + '</option>').trigger('change').select2("enable",false);
				$('#gol_darah').html('<option value="' + data.gol_darah + '">' + data.gol_darah + '</option>').trigger('change').select2("enable",false);
				$('#identitas').html('<option value="' + data.identitas + '">' + data.identitas + '</option>').trigger('change').select2("enable",false);

				// $('#asuransi_utama').html('<option value="' + data.asuransi_utama + '">' + data.asuransi_utama + '</option>').trigger('change');

				
				$('#provinsi').html('<option value="'+data.provinsi+'">'+data.provinsi+'</option>').trigger('change').select2("enable",false);
				$('#kabupaten').html('<option value="'+data.kabupaten+'">'+data.kabupaten+'</option>').trigger('change').select2("enable",false);
				$('#kecamatan').html('<option value="'+data.kecamatan+'">'+data.kecamatan+'</option>').trigger('change').select2("enable",false);
				$.each(data, function (key, val) {
					if (key != 'no_asuransi') {
						$div_body.find("[name='" + key + "']").val(val).attr({ readonly: '', title: 'Bisa diedit di menu data pasien' });
					} else {
						$div_body.find("[name='default_" + key + "']").val(val);
						$div_body.find("[name='default_asuransi']").val(data.asuransi_utama);
						$div_body.find("[name='default_nama_asuransi']").val(data.nama_asuransi_utama);
...
$('#autocode').on("click", function () {
	http_request('pendaftaran/searchcode', 'GET', { id: $(this).data('id') })
		.done(function (result) {
			$("form").get(0).reset()
			console.log("selcet ",$select2_disabled)
			if ($select2_disabled) {
				$('#jenis_kelamin').select2('enable', [true])
				$('#status_nikah').select2('enable', [true])
				$('#agama').select2('enable', [true])
				$('#gol_darah').select2('enable', [true])
				$('#identitas').select2('enable', [true])

				// $('#asuransi_utama').html('<option value="' + data.asuransi_utama + '">' + data.asuransi_utama + '</option>').trigger('change');

			
				$('#provinsi').select2('enable', [true])
				$('#kabupaten').select2('enable', [true])
				$('#kecamatan').select2('enable', [true])
			}
			var data = result;
			$("#nomor_rm").val(data);
...
$("#asuransi_utama").select2({
	...
	$("#no_asuransi").val('')
	if(data.id==$("#default_asuransi").val()) $("#no_asuransi").val($("#default_no_asuransi").val())
