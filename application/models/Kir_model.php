...
class Kir_model extends CI_Model
{
	...
	function _search($antrian, $pasien)
	{
		$data = $this->db->query("
			...
			IFNULL(GROUP_CONCAT(DISTINCT diag.namaDiagnosis SEPARATOR '|'),'') as diagnosa
			FROM 
			...
			LEFT JOIN tbl_pemeriksaan_diagnosa p_diag ON periksa.id_pemeriksaan=p_diag.fk_pemeriksaan
