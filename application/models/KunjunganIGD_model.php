...
class KunjunganIGD_model extends CI_Model
{
	...
	{
		$this->db->select('pasien.id_pasien as _id,pasien.*, fix_agama.nama_agama as agama, tbasuransi.namaAsuransi as nama_asuransi_utama,fix_status_pernikahan.nama_status_pernikahan as status_nikah,fix_gol_darah.nama_gol_darah as gol_darah,wilayah_provinsi.nama as provinsi,wilayah_kabupaten.nama as kabupaten,wilayah_kecamatan.nama as kecamatan');
	...
	function get_no_invoice($clinic_id)
	{
		date_default_timezone_set('Asia/Jakarta');
		// $this->db->select('RIGHT(tbl_pendaftaran.no_invoice,3) as no_invoice');
		$this->db->select('IFNULL(MAX(CAST(REGEXP_SUBSTR(no_invoice,"[0-9]+$") AS UNSIGNED)),0) as no_invoice');
		$this->db->order_by('no_invoice', 'DESC');
		$this->db->limit(1);
		$this->db->where("clinic_id", $clinic_id);
		$this->db->like('CAST(REGEXP_SUBSTR(no_invoice,"[0-9]+$") AS UNSIGNED)', date("ymd"),"after");
		$query = $this->db->get('tbl_pendaftaran');
		if ($query->num_rows() <> 0) {
			$data = $query->row();
			if(intval($data->no_invoice)==0){
				$kode = date("ymd")."001";
			}else{
				$kode = intval($data->no_invoice) + 1;
			}
