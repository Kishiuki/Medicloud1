...
class Antrian_pemeriksaan_model extends CI_Model
{
	...
	{
		$orders_cols = ["antrian.id_antrian", "antrian.nomor_antrian", "pendaftaran.id_asuransi", "pendaftaran.no_asuransi", "pasien.nomor_rm", "pasien.nama_lengkap", "pasien.alamat", "poliklinik.namaPoli", "asuransi.namaAsuransi", "pendaftaran.no_invoice"];
		...
		$limit = 0;
		$offset = 25;
		$data = $this->db->query("SELECT SQL_CALC_FOUND_ROWS " . implode(",", $orders_cols) . ",pasien.id_pasien FROM tbl_antrian AS antrian 
		LEFT JOIN tbl_pendaftaran AS pendaftaran ON antrian.id_antrian = pendaftaran.fk_antrian 
		LEFT JOIN tbl_pasien AS pasien ON pendaftaran.fk_pasien = pasien.id_pasien 
		LEFT JOIN tbpoli as poliklinik ON pendaftaran.poli = poliklinik.idPoli 
		LEFT JOIN tbasuransi AS asuransi ON pendaftaran.id_asuransi = asuransi.idAsuransi 
		...
		$map_data = array_map(function ($dt) {
			...
			return [
				...
				$dt->no_invoice,
				$dt->nomor_antrian . 
				'<div class="btn-group"> <a href="#" class="ml-1 btn btn-danger btn-xs link-lewati" data-dismiss="modal" data-id="' . $dt->id_antrian . '" data-poli="'.$dt->namaPoli.'">Lewati</a>'.
				' <span>&nbsp;</span><a href="#" class="btn btn-success btn-xs link-panggil-antrian" data-dismiss="modal" data-nomor="'.$dt->nomor_antrian.'"  data-poli="'.$dt->namaPoli.'" data-id="' . $dt->id_antrian . '"><i class="fa fa-bullhorn"></i> Panggil</a>
				</div>',
				$dt->namaPoli,
				$dt->namaAsuransi,
				$dt->no_asuransi,
