...
class Data_pasien_model extends CI_Model
{
	...
	function _load_dt($posted)
	{
		...
		if(isset($posted['asuransi']) && $posted['asuransi']!="" && $posted['asuransi']!=null){
			$sWhere .= ($sWhere == "") ? " WHERE " : " AND ";
			if($posted['asuransi']=="n"){
				$posted['asuransi']="";
			}
			$sWhere .= " pasien.asuransi_utama='".htmlentities($posted['asuransi'])."'";
		}
		$sLimit = $output->limit;
		$sGroup = "";
		$sOrder = $output->order;
	...
	{
		$data = $this->db->query('select id_pasien as _id,nomor_rm,nama_lengkap,pasien.*,pend.*,pend.create_at as tanggal_kunjungan,pem.*,
		IFNULL(GROUP_CONCAT(DISTINCT diag.namaDiagnosis SEPARATOR "|"),"") as diagnosa,IFNULL(GROUP_CONCAT(DISTINCT layanan_poli.nama_layanan_poli SEPARATOR "|"),"") as tindakan, kec.nama as nama_kecamatan,kab.nama as nama_kabupaten,prov.nama as nama_provinsi,
		IFNULL(GROUP_CONCAT(DISTINCT CONCAT(obat.nama," (",resep.qty,")") ORDER BY resep.resep_detail_id SEPARATOR "|"),"") as resep
		FROM tbl_pasien pasien 
		INNER JOIN tbl_pendaftaran pend ON pasien.id_pasien=pend.fk_pasien 
		...
		LEFT JOIN tbl_resep_detail resep ON pend.id_pendaftaran=resep.fk_pendaftaran AND resep.status=1 
		LEFT JOIN tbl_obat obat ON resep.fk_obat=obat.idObat
