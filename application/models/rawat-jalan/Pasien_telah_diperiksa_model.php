...
class Pasien_telah_diperiksa_model extends CI_Model
{
	...
	function _load_dt($posted)
	{
		...
		$limit = 0;
		$offset = 25;
		$data = $this->db->query("SELECT SQL_CALC_FOUND_ROWS " . implode(",", $orders_cols) . "  FROM tbl_antrian AS antrian 
	...
	function tampilkan_temp($clinic_id, $id_pendaftaran)
	{
		...
		$query = "SELECT o.kode,o.nama,s.namaSatuanobat,td.*,aturan.nama_aturan_pakai,cara.nama_cara_pakai
         ...
         WHERE o.idObat=td.fk_obat and td.satuan=de.obat_detail_id and de.satuan=s.idSatuanobat and aturan.id=td.aturan_pakai and cara.id=td.cara_pakai and td.status=0 and td.clinic_id='$clinic_id' AND td.fk_pendaftaran='$id_pendaftaran'";
	...
	{
		$this->db->query("UPDATE tbl_resep_detail SET fk_pendaftaran='$id' ,status='1' where fk_pendaftaran='$id' ");
	}
