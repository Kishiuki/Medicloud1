...
class Billing_model extends CI_Model
{
	...
	function _load_dt($posted)
	{
		...
		if(isset($posted['asuransi']) && $posted['asuransi']!=null && $posted['asuransi']!=""){
			$sWhere .= ($sWhere == "") ? " WHERE " : " AND ";
			$sWhere .= " pendaftaran.id_asuransi='".htmlentities($posted['asuransi'])."' ";
		}
		...
		$limit = 0;
		$offset = 25;
		$data = $this->db->query("SELECT SQL_CALC_FOUND_ROWS " . implode(",", $orders_cols) . ",asuransi.namaAsuransi FROM tbl_pendaftaran AS pendaftaran 
	...
	{
		$this->db->select('tbl_pendaftaran.id_pendaftaran as _id,tbl_pendaftaran.id_pendaftaran as id_pend,tbl_pasien.asuransi_utama,tbl_pendaftaran.id_asuransi, tbl_pasien.nomor_rm, tbl_pasien.nama_lengkap, tbl_pendaftaran.create_at,tbl_pendaftaran.no_invoice');
