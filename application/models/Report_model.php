...
class Report_model extends CI_Model
{
	...
	function _load_dt_kunjungan_pasien($posted,$is_export)
	{
		...
		if(isset($posted['poli']) && $posted['poli']!="" && $posted['poli']!=null){
			$sWhere .= ($sWhere == "") ? " WHERE " : " AND ";
			$sWhere.=" pendaftaran.poli='".htmlentities($posted['poli'])."'";
		}
		if(isset($posted['dokter']) && $posted['dokter']!="" && $posted['dokter']!=null){
			$sWhere .= ($sWhere == "") ? " WHERE " : " AND ";
			$sWhere.=" tindakan.fk_dokter='".htmlentities($posted['dokter'])."'";
		...
		$limit = 0;
		$offset = 25;
		// 2022-10-31: fix issue filter poli, salah relasi (relasi poli diambil dari pendaftaran)
		$data = $this->db->query("SELECT SQL_CALC_FOUND_ROWS " . implode(",", $orders_cols) . ",DATE_FORMAT(pemeriksaan.create_at,'%Y-%m-%d') as tgl_periksa,pasien.nomor_rm,pendaftaran.clinic_id,tindakan.fk_dokter,GROUP_CONCAT(DISTINCT diagnosis.namaDiagnosis ORDER BY diagnosis.idDiagnosis SEPARATOR '; ') as nama_diagnosis,
        ...
        LEFT JOIN tbl_layanan_poli lay_poli ON tindakan.fk_tindakan=lay_poli.id_layanan_poli
				LEFT JOIN tbpoli poli ON pendaftaran.poli=idPoli
    $sWhere $sGroup $sOrder $sLimit")->result_object();
		$found = $this->db->query("SELECT FOUND_ROWS() as total")->result_object();
		if($is_export) return $data;
		$map_data = array_map(function ($dt) {
    ...
    $sWhere $sGroup ORDER BY total DESC ")->result_object();
		$found = $this->db->query("SELECT FOUND_ROWS() as total")->result_object();
		if($is_export) return $data;
		$map_data = array_map(function ($dt) {
			// $id_pemeriksaan = $dt->id_pemeriksaan;
