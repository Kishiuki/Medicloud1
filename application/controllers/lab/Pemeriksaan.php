...
class Pemeriksaan extends CI_Controller
{
	...
	public function load_dt()
	{
		...
		$posted=($meth=='POST') ? $this->input->input_stream() : $this->input->get();
		if($meth=='POST') header('Content-Type: application/json');
		$posted = modify_post($posted);
		$provider_id = ($this->data['C_PV_GROUP'] != "pusat") ? $this->data['C_PV_ID'] : '';
		
		$is_export=(isset($posted['export'])) ? TRUE : FALSE;
		$data = $this->pemeriksaan->_load_dt($posted, $provider_id,$is_export);
		if(!$is_export){
			echo json_encode($data);
			die();
		}
		if($is_export){
			$page_title="Export ".date("Y-m-d His");
			...
			$row = 1;
			$sheet->setCellValue("A" . $row, 'Tgl Daftar');
			$sheet->setCellValue("B" . $row, 'Tgl Sampling');
			$sheet->setCellValue("C" . $row, 'No RM');
			$sheet->setCellValue("D" . $row, 'NIK');
			$sheet->setCellValue("E" . $row, 'Nama Pasien');
			$sheet->setCellValue("F" . $row, 'Jenis Kelamin');
			$sheet->setCellValue("G" . $row, 'Usia');
			$sheet->setCellValue("H" . $row, 'Pemeriksaan');
			$sheet->setCellValue("I" . $row, 'Status');
			$sheet->setCellValue("J" . $row, 'Biaya');
			$sheet->setCellValue("K" . $row, 'Bayar');
			$sheet->setCellValue("L" . $row, 'User');
			$sheet->setCellValue("M" . $row, 'Update');
			...
			foreach ($data as $dt) {
				...
				$sheet->setCellValue("A" . $row, $dt->tgl_periksa);
				$sheet->setCellValue("B" . $row, $dt->tgl_sampling);
				$sheet->setCellValue("C" . $row, $dt->no_pasien);
				$sheet->setCellValue("D" . $row, $dt->no_identitas);
				$sheet->setCellValue("E" . $row, $dt->nama_pasien);
				$sheet->setCellValue("F" . $row, $dt->jenis_kelamin);
				$sheet->setCellValue("G" . $row, $dt->usia);
				$sheet->setCellValue("H" . $row, $dt->jenis_pemeriksaan);
				$sheet->setCellValue("I" . $row, strtoupper($dt->status));
				$sheet->setCellValue("J" . $row, format_number($dt->biaya));
				$sheet->setCellValue("K" . $row, format_number($dt->bayar));
				$sheet->setCellValue("L" . $row, $dt->create_by);
				$sheet->setCellValue("M" . $row, $update_at);
				$row++;
				$n++;
			}
			$row--;
			$sheet->getStyle("A$start_row_content:M$row")->applyFromArray($styleBorder);
	...
	public function export_data()
	{
		...
		$posted = modify_post($posted);
		$provider_id = ($this->data['C_PV_GROUP'] != "pusat") ? $this->data['C_PV_ID'] : '';
		$data = $this->pemeriksaan->_load_dt($posted, $provider_id,TRUE);
		print_r($data);
	}
	...
	public function save__()
	{
		...
		if (!isset($posted['ref_id']) || $posted['ref_id'] == "") {
			...
			$save = $this->pemeriksaan->_save(array(
				...
				"asuransi" => (isset($asuransi)) ? $asuransi : "",
				"no_asuransi"   => (isset($no_asuransi)) ? $no_asuransi: 0,
				"perujuk"       => (isset($perujuk)) ? $perujuk: "",
				"nama_tenaga_perujuk"   => (isset($nama_tenaga_perujuk)) ? $nama_tenaga_perujuk : "",
			), array());
