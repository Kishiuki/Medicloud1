		...
		<div class="card-header border-bottom bg-success">
			...
			<?php if (!empty($list_by_provider)) { ?>
				<div class="btn-group pull-right">
					...
					<a href="<?= base_url(conf('path_module_lab') . 'report/by-periode/export/excel?clinic_id='.$clinic_id.'&periode=' . $selected_year . '-' . $selected_month); ?>" target="_blank" class="btn btn-xs bg-white pull-right"><i class="fa fa-file-excel-o tx-success"></i> Download Excel</a>
					<a href="<?= base_url(conf('path_module_lab') . 'report/by-periode/export/pdf?clinic_id='.$clinic_id.'&periode=' . $selected_year . '-' . $selected_month) . "&pdf=true"; ?>" target="_blank" class="btn btn-xs pull-right bg-white"><i class="fa fa-file-pdf-o tx-danger"></i> Download PDF</a>
				</div>
			<?php } ?>
		</div>
